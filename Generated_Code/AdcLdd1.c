...
**     Component   : ADC_LDD
**     Version     : Component 01.138, Driver 01.08, CPU db: 3.00.000
...
**          Single conversion time - Single-ended          : 5.208 us
**          Single conversion time - Differential          : 6.708 us
**          Additional conversion time - Single-ended      : 4.166 us
**          Additional conversion time - Differential      : 5.666 us
...
**         <CreateSampleGroup> method. The <OnMeasurementComplete() >
**         event is invoked after the measurement is done and the event
**         is enabled. The state of the measurement can be also polled
**         by the <GetMeasurementCompleteStatus()> method. Results of
**         the measurement can be read by the <GetMeasuredValues()>
**         method. The <Discontinuous mode> doesn't support this method.
**     @param
...
bool AdcLdd1_GetMeasurementCompleteStatus(LDD_TDeviceData *DeviceDataPtr)
{
  ...
  EnterCritical();
  Status = ((AdcLdd1_TDeviceDataPtr)DeviceDataPtr)->CompleteStatus; /* Save flag for return */
  ((AdcLdd1_TDeviceDataPtr)DeviceDataPtr)->CompleteStatus = FALSE; /* Clear measurement complete status flag */
